...
from naive.lazy_worker_naive import NaiveMaliciousWorker
from naive.naive_success_listener import NaiveCertificationSuccessListener
from naive.normal_worker_naive import NaiveNormalWorker
...
def start_worker(worker_id, ecs, kill_all, run_args, client_type: ClientType):
    if client_type == ClientType.NORMAL:
        node = NaiveNormalWorker(worker_id, ecs, kill_all, run_args)  # , io.StringIO())
    else:
        node = NaiveMaliciousWorker(
            worker_id, ecs, kill_all, run_args
        )  # , io.StringIO())
...
def create_listener(listener_id, data, listener_type: ListenerType):
    ...
    match listener_type:
        ...
        case ListenerType.CERT_SUCCESS:
            node = NaiveCertificationSuccessListener(
...
def initialize_nodes(data, run_args):
    ...
    ) or scenario == ExperimentScenarios.MaliciousUser:
    ...
    listener_type = ListenerType.CERT_FAILURE
    verifier_nodes_fail = start_listeners(
        node_config.getint("Verifiers"), data, listener_type
    )
    verifier_nodes.extend(verifier_nodes_fail)
